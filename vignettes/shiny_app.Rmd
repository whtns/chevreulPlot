...
plot_colData_histogram((small_example_dataset), group_by = "nFeature_gene", fill_by = "Mutation_Status")
plot_colData_histogram((small_example_dataset), group_by = "nCount_gene", fill_by = "Mutation_Status")
```
...
de_input <- chevreulProcess::run_object_de(tiny_sce, 
colnames(tiny_sce)[1:100], colnames(tiny_sce)[101:200], diffex_scheme = "custom", tests = "t")
```
...
thresholded log fold change.
 ...
 ```{r}  
chevreulPlot::plot_marker_features(small_example_dataset, group_by = "gene_snn_res.1")
```
...
transcript make for each gene expression?
 ...
 ```{r}  
data(tiny_sce)
chevreulPlot::plot_transcript_composition(tiny_sce, "NRL")
```
